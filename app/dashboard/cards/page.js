...
import { CreditCard, Plus, Snowflake, Trash2, Eye, EyeOff } from 'lucide-react';
...
export default function CardsPage() {
  ...
  const fetchCards = async () => {
    try { const { data } = await api.get('/cards'); setCards(data.data); }
  ...
  const createCard = async () => {
    setCreating(true);
    try { await api.post('/cards'); toast.success('Virtual card created'); fetchCards(); }
    catch (err) { toast.error(err.response?.data?.message || 'Failed'); }
    finally { setCreating(false); }
  };

  const toggleFreeze = async (id) => {
    try { const { data } = await api.patch(`/cards/${id}/freeze`); toast.success(data.message); fetchCards(); }
    catch { toast.error('Action failed'); }
  };

  const cancelCard = async (id) => {
    if (!confirm('Cancel this card permanently?')) return;
    try { await api.delete(`/cards/${id}`); toast.success('Card cancelled'); fetchCards(); }
  ...
  return (
    <div className="p-6 lg:p-8 animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <p className="text-xs font-semibold tracking-widest mb-1" style={{ color: 'rgba(255,106,0,0.7)' }}>PAYMENT INSTRUMENTS</p>
          <h1 className="font-display text-2xl font-bold text-white">Virtual Cards</h1>
        </div>
        <button onClick={createCard} disabled={creating || cards.filter(c => c.status !== 'CANCELLED').length >= 3}
          ...
          {creating ? <div className="w-3.5 h-3.5 border-2 border-black border-t-transparent rounded-full animate-spin" /> : <Plus size={14} />}
          Issue Card
        </button>
      </div>

      {loading ? (
        <div className="grid md:grid-cols-2 gap-5">{[1,2].map(i => <div key={i} className="h-48 rounded-2xl skeleton" />)}</div>
      ) : cards.length === 0 ? (
        <div className="text-center py-20 rounded-2xl" style={{ background: '#111111', border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          <h3 className="font-display font-semibold text-white mb-2">No cards issued</h3>
          <p className="text-sm mb-5" style={{ color: 'rgba(255,255,255,0.3)' }}>Issue your first virtual card</p>
          <button onClick={createCard} className="btn-primary px-6 py-2.5 rounded-xl text-sm font-semibold">Issue Card</button>
        </div>
      ) : (
                  ...
                  {frozen && (
                    <div className="absolute inset-0 flex items-center justify-center z-10">
                      <div className="flex items-center gap-2 px-4 py-2 rounded-full" style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(129,140,248,0.3)' }}>
                        ...
                        <span className="text-xs font-semibold" style={{ color: '#818cf8' }}>Card Frozen</span>
                      </div>
                    </div>
                  )}
                  <div className="relative z-10 p-5 h-full flex flex-col justify-between">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.15)' }}>{card.network} · {card.type}</p>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex justify-between items-end">
                        <div>
                          <p className="text-xs mb-0.5" style={{ color: 'rgba(255,255,255,0.2)' }}>HOLDER</p>
                          <p className="text-xs font-semibold tracking-wide text-white">{card.cardHolder}</p>
                        </div>
                        <div>
                          <p className="text-xs mb-0.5" style={{ color: 'rgba(255,255,255,0.2)' }}>EXPIRES</p>
                          <p className="text-xs font-semibold text-white">{String(card.expiryMonth).padStart(2,'0')}/{String(card.expiryYear).slice(-2)}</p>
                        </div>
                        <div>
                          <p className="text-xs mb-0.5" style={{ color: 'rgba(255,255,255,0.2)' }}>CVV</p>
                          <p className="text-xs font-semibold text-white font-mono">{show ? card.cvv : '•••'}</p>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <div className="mt-5 p-4 rounded-xl text-xs" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.2)' }}>
        Virtual cards are simulated instruments. Maximum 3 active cards per account.
      </div>
    </div>
  );
}
